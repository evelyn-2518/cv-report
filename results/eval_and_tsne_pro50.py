...
# 2. 模型檔案名稱
MODEL_FILENAME = '/Users/winny/Desktop/電腦視覺/cv_final_project/pro-50/baseline_model.pth'
MODEL_PATH = MODEL_FILENAME 

# 3. 輸出圖片路徑 (改名以區別)
TSNE_IMAGE_PATH = os.path.join(SCRIPT_DIR, 't_sne_baseline_pca_fix.png')

# 4. 可視化參數
MAX_TSNE_SAMPLES = 3000   # 抽樣點數
PCA_COMPONENTS = 50       # PCA 先降到 50 維
TSNE_PERPLEXITY = 30      # t-SNE 參數 (30-50)
...
def load_baseline_model(path: str) -> nn.Module:
    print(f"正在載入 Baseline 模型: {path}")
...
def main():
    ...
    # PCA 降維 (使用 randomized solver 加速並避免卡死)
    ...
    # t-SNE
    print(f"正在執行 t-SNE (Input Dim={PCA_COMPONENTS})...")
    time_tsne = time.time()
    ...
    plt.title(f"t-SNE (Baseline, PCA={PCA_COMPONENTS}, Perp={TSNE_PERPLEXITY})")
    plt.axis('off')
